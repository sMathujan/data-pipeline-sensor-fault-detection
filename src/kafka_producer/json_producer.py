...
def product_data_using_file(topic,file_path):
    ...
    schema_str = Generic.get_schema_to_produce_consume_data(file_path=file_path) # produce schema for given file path
    schema_registry_conf = schema_config() # return the schema configs
    schema_registry_client = SchemaRegistryClient(schema_registry_conf) # function from confluent
    string_serializer = StringSerializer('utf_8')
    json_serializer = JSONSerializer(schema_str, schema_registry_client, instance_to_dict) # instance_to_dict converts the instance to dictonary
    producer = Producer(sasl_conf()) #Producer from confluent kafka

    print("Producing user records to topic {}. ^C to exit.".format(topic))
    # while True:
    # Serve on_delivery callbacks from previous calls to produce()
    producer.poll(0.0) #Whenever we send data, it makes sure every data produced successfully
    try:
